...
const Navbar = ({ user }) => {
  ...
  useEffect(() => {
    if (user) {  // Assurez-vous que 'user' est défini avant de faire l'appel API
    ...
    <nav className="dashboard-nav-gradient text-white shadow-lg" style={{ width: '100%' }}>
      <div className="dashboard-nav-container">
        ...
        <div className="dashboard-right-menu">
          ...
          />
          {/* Menu utilisateur */}
          <UserDropdown user={user} initials={`${user.nom.charAt(0).toUpperCase()}${user.prenom.charAt(0).toUpperCase()}`} />
        </div>
      </div>
